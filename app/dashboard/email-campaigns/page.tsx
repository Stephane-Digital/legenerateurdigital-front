...
type RealActionRecord = {
  ...
  date: string;
  createdAt: string;
  module: "emailing" | "editor" | "lead_engine" | "coach" | "planner" | "library";
  action: string;
  label: string;
  title: string;
  details?: string;
...
function readMissionCashActionFromPayload() {
  ...
  try {
    ...
    const raw = window.localStorage.getItem(CMO_AUTO_PAYLOAD_KEY) || window.localStorage.getItem(CMO_DISPATCH_PAYLOAD_KEY);
    if (!raw) return null;
...
function recordEmailingRealAction(title: string, details = "Séquence email générée dans Emailing IA.") {
  if (typeof window === "undefined") return;
  ...
  writeDashboardProgressPatch({ idea: true, email: true });
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 2xl:grid-cols-[1.05fr_0.95fr]">
          <EmailCampaignGenerator
            ...
            onGenerated={(nextSequence) => {
              const sanitized = sanitizeEmailSequenceResponse(nextSequence);
              setSequence(sanitized);
              recordEmailingRealAction(values.offer_name || values.sender_name || "Campagne email générée");
            }}
